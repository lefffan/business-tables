...
if (isset($_SERVER['argv'][1])) switch($_SERVER['argv'][1])
   {
    ...
    case 'SETPROP':
	 ...
	 if ($data['altkey'] || $data['ctrlkey'] || $data['metakey'] || $data['shiftkey']) break;
	 if (($len = count($_SERVER['argv'])) < 4) break;
	 $profile = [];
	 $margin = "\n";
	 for ($i = 3; $i < $len; $i += 2)
	     {
	      ...
	      $profile[$i] = ['type' => 'textarea', 'head' => $margin."Enter element '$prop' property value:", 'data' => $value, 'line' => ''];
	      $margin = '';
	     }
