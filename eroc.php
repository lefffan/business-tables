...
function getUserCustomization($db, $id)
{
 global $odid, $oid, $eid;
 $odid = '1';
 $oid = $id;
 $eid = '6';
 return getElementProperty($db, 6, 'dialog');
}

function getUserCustomization1($db, $id)
{
 $query = $db->prepare("SELECT JSON_EXTRACT(eid6, '$.dialog') FROM `data_1` WHERE id=:id AND eid6 IS NOT NULL ORDER BY version DESC LIMIT 1");
 $query->execute([':id' => $id]);
 $name = $query->fetchAll(PDO::FETCH_NUM);
 if (isset($name[0][0])) return substr($name[0][0], 1, -1);
}
...
function defaultCustomizationDialogJSON($name)
{
 ...
 $dialog = ['pad' => []];
 $dialog['pad']['Scheme'] = ['element2' => ['type' => 'text', 'head' => 'Force to use next user scheme (empty or non-existent user - current scheme is used):', 'data' => '', 'line' => '']];
